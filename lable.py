...
class Lable(pygame.sprite.Sprite):
	...
	def __init__(self, parent:pygame.sprite.Sprite, pos:Tuple[int, int],
				font:Union[FontProperty, dict, list, None], text:str,
				background:Tuple[int, int, int], text_align:str,
				trancparency:bool=False,
				rect_size:Union[list, tuple]=None,
				size_range:SizeRange=None, padding:Padding=Padding(5),
				border:int=4,
	...
	def get_rect(self)->pygame.Rect:
		...
		pygame.Rect
		    Description
		"""
		return pygame.Rect(self.client_rect.x - self.margin.left,
		...
		for line in self.text.splitlines():
			if self.size_range is None or self.size_range.max_w is None:
				...
				while len(line) != 0\
						...
						- Padding.ansolute_horizontal_indent(self.padding, self.border):
	...
	def calculate_collide(self, lables:list, unmovable_x:bool=False,
						unmovable_y:bool=False)->int:
	...
	@staticmethod
	def _collide(self_rect:pygame.Rect, collided_lables:list)->Generator[Any, int, int]:
		...
		for lable in collided_lables:
			if self_rect.colliderect(lable.get_rect):
				...
				delta_x = min(self_rect.w, lable_rect.w) - abs(self_rect.x - lable_rect.x)
				delta_y = min(self_rect.h, lable_rect.h) - abs(self_rect.y - lable_rect.y)
	...
	def get_rect_align(self, size:Tuple[int, int], line_number:int)->pygame.Rect:
		...
		"""
		if self.align[0] == 'r':
			rect = pygame.Rect((self.client_rect.w-self.padding.rigth-self.border.rigth-size[0],
								self.padding.top + self.border.top + size[1]*line_number),
								size)
		elif self.align[0] == 'c':
			rect = pygame.Rect((self.client_rect.w//2 - size[0]//2,
								self.padding.top + self.border.top + size[1]*line_number),
								size)
		else:
			rect = pygame.Rect((self.padding.left + self.border.left,
								self.padding.top + self.border.top + size[1]*line_number),
								size)
		# pdb.set_trace()
		return rect
	...
	def draw_text(self)->None:
		...
		for line in self.text.splitlines():
			line_counter += 1
			if self.font.size(line)[0] <= self.client_rect.width \
				- Padding.absolute_horizontal_indent(self.padding, self.border):
				...
				line = line.split()
				while len(line) != 0\
					 or self.font.size(' '.join(line))[0] > self.client_rect.width\
					 - Padding.absolute_horizontal_indent(self.padding, self.border):
					while self.font.size(' '.join(line))[0] > self.client_rect.width \
							- Padding.absolute_horizontal_indent(self.padding, self.border):
						buffer.append(line.pop())
					# pdb.set_trace()
