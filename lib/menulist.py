...
from .modules import Padding, SizeRange
...
class MenuList(pygame.sprite.Sprite, IEventBound):
    ...
    def __init__(
        ...
        self._client_rect = pygame.Rect(pos, (0, 0))
    ...
    def add(self, item):
        if isinstance(item, IEventBound) or not isinstance(item, EventlessButton):
    ...
    def _add_to_vertical(self, item):
        ...
        item.client_rect.y = self._client_rect.height
    ...
    def _add_to_horizontal(self, item):
        ...
        item.client_rect.x = self._client_rect.width
        self._client_rect.width += new_item_size[0]
        if not self._may_reset_items_rect(new_item_size):
            item.client_rectangle = pygame.Rect(item.client_rect.topleft, new_item_size)

    def _remove_from_horizontal(self, count):
        self._client_rect.width -= self.item_size[0]
        for iter_, item in enumerate(self.items):
            item.id = iter_
            item.client_rect.x = self.item_size[0] * iter_ + item.margin.left

    def _remove_from_vertical(self, count):
        self._client_rect.height -= self.item_size[1] * count
        for iter_, item in enumerate(self.items):
            item.id = iter_
            item.client_rect.y = self.item_size[1] * iter_ + item.margin.top

    def remove(self, item_id):
        self.items.pop(item_id)
        try:
            getattr(self, '_remove_from_' + self.orientation.name.lower())()
        except AttributeError:
